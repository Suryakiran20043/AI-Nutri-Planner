...
const GOAL_ADJUST = { lose: -500, maintain: 0, gain: 300, bodybuild: 200 };
const GOAL_LABEL  = {
  lose    : 'Weight loss (−500 kcal/day)',
  maintain: 'Maintenance',
  gain    : 'Weight gain (+300 kcal/day)',
  bodybuild: 'Bodybuilding (+200 kcal/day)',
};
...
function runCalculator() {
  ...
  let p_ratio = 0.30, c_ratio = 0.40, f_ratio = 0.30;
  if (selectedGoal === 'bodybuild') { p_ratio = 0.40; c_ratio = 0.35; f_ratio = 0.25; }
  else if (selectedGoal === 'lose') { p_ratio = 0.35; c_ratio = 0.35; f_ratio = 0.30; }
  else if (selectedGoal === 'gain') { p_ratio = 0.25; c_ratio = 0.50; f_ratio = 0.25; }

  const proteinG  = Math.round(dailyCal * p_ratio / 4);
  const carbsG    = Math.round(dailyCal * c_ratio / 4);
  const fatG      = Math.round(dailyCal * f_ratio / 9);
